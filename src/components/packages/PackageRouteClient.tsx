...
export default function PackageRouteClient({ id }: { id: string }) {
  ...
  if (!pkg) {
    return <main className="min-h-screen flex items-center justify-center bg-sand"><div className="text-center"><p className="text-sm font-extrabold uppercase tracking-widest text-accent">404</p><h1 className="mt-2 text-3xl font-heading font-extrabold text-primary">Package not found</h1><p className="mt-3 text-sm text-foreground-muted">This itinerary may have been removed or is not published yet.</p></div></main>;
  }
